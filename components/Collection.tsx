        ...
        <div className="mt-16 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {products.map((p, i) => (
            <Reveal key={p.slug} delay={(i % 3) * 0.08} y={36}>
              <article className="group relative flex h-full flex-col overflow-hidden rounded-[1.75rem] border border-white/60 bg-gradient-to-b from-ivory-50 to-champagne-light/70 transition-all duration-700 ease-luxe hover:-translate-y-1.5 hover:shadow-[0_50px_90px_-50px_rgba(46,59,44,0.6)]">
                ...
                <div className="relative h-64 overflow-hidden">
                  {p.bottle ? (
                    <>
                      ...
                      <div className="absolute inset-0 flex items-center justify-center">
                        <Image
                          ...
                          width={260}
                          height={360}
                          className="h-auto w-[270px] drop-shadow-[0_26px_34px_rgba(46,59,44,0.35)] transition-transform duration-700 ease-luxe group-hover:-translate-y-2 group-hover:scale-105"
                        />
                      </div>
                    </>
                  ) : (
                    <>
                      <Image
                        src={p.botanicalImage}
                        alt={`${p.name} — ${p.botanical}`}
                        fill
                        sizes="(max-width: 640px) 92vw, (max-width: 1024px) 46vw, 30vw"
                        className="object-cover transition-transform duration-[1400ms] ease-luxe group-hover:scale-105"
                      />
                      <div className="absolute inset-0 bg-gradient-to-t from-ivory-50/90 via-transparent to-transparent" />
                    </>
                  )}
                  {/* index + type */}
                ...
                <div className="flex grow flex-col border-t border-botanical/10 px-6 py-5">
                  <div className="flex items-baseline justify-between gap-3">
                    <h3 className="font-display text-3xl leading-none text-botanical-deep">
                      {p.name}
                    </h3>
                  ...
                  <p className="mt-2 text-sm italic text-stone-mid">
                    {p.botanical}
                  </p>
