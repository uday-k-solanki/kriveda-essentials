...
const FILTER_TABS: { id: Filter; name: string }[] = [
  { id: "all",       name: "All Oils" },
  { id: "essential", name: "Essential Oils" },
  { id: "carrier",   name: "Carrier Oils" },
];

// Pricing and meta come from the central product-config
...
export default function CatalogueContent() {
  ...
  // Merge Shopify live data with config pricing
  const products = useMemo(() => {
    return staticProducts.map((p) => {
      const shopify = shopifyProducts.find((s) => s.slug === p.slug);
      const meta    = PRODUCT_CONFIG[p.slug];
      return {
        ...p,
        variantId:        shopify?.variantId ?? "",
        availableForSale: shopify?.availableForSale ?? false,
        originalPrice:    meta?.originalPrice   ?? p.price,
        discountedPrice:  meta?.discountedPrice  ?? p.price,
        discountLabel:    meta?.discountLabel    ?? "",
        isBestseller:     meta?.isBestseller     ?? false,
        category:         p.type === "Essential Oil" ? "essential" : "carrier",
        bottle:           (p as typeof p & { bottle?: string }).bottle,
      };
    });
  }, [shopifyProducts]);
                      ...
                      <div className="flex grow flex-col border-t border-botanical/10 px-6 py-5">
                        ...
                        <div className="mt-3">
                          <div className="flex items-baseline gap-2">
                            <span className="text-[1.4rem] font-bold leading-none text-botanical-deep">₹{p.discountedPrice}</span>
                            <span className="text-sm leading-none text-stone-mid line-through">₹{p.originalPrice}</span>
                            <span className="rounded-full bg-gold/15 px-1.5 py-0.5 text-[0.52rem] font-semibold text-gold-deep">{p.discountLabel}</span>
                          </div>
